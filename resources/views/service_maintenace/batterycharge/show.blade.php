           ...
           <div class="col-sm-4 col-md-4">
             <form id="target" class="pull-right" action="{{ route('batterycharge.import') }}" method="POST" enctype="multipart/form-data">
                 ...
                 <div class="file btn btn-inverse"><i class="fas fa-file-download"></i>
                  ...
                  <a class="btn btn-inverse" href="{{route('batterycharge.download') }}"><i style="margin-right: 5px; " class="fas fa-file-import"></i></i>Format</a>
            ...
            <table id="myTable">
              ...
              @php ($vch_no = \App\vehicle_master::find($Battery->vch_id))
                ...
                <tr>
                  ...
                  <td style="width:20%; padding-left: 20px">{{$Battery->date}}</td>
                  <td style="width:20%; padding-left: 20px">{{$Battery->km_reading}}</td>
                  <td style="padding-left: 20px">{{$Battery->chr_by}}</td>
                   <td style="padding-left: 20px">{{$Battery->cost}}</td>
                  <td style="width:10%; text-align:center;">
                    <a href="{{route('batterycharge.edit',$Battery->id)}}" runat="server" class="btn btn-success" rel="tooltip" title="" data-original-title="Edit"><i class="fa fa-edit"></i></a>
                    <a onclick="javascript:return confirm('Do You Really Want To Delete This?');" href="{{route('batterycharge.delete',$Battery->id)}}" class="btn btn-inverse" rel="tooltip" title="" data-original-title="Delete"><i class="fa fa-times"></i></a>
                  </td>
                </tr>
                @endforeach
